...
---Cloude module for Steam plugin development
---@class cloude
local cloude = {}
...
---Used to let Cloude know what plugins crashed/loaded etc.
---@return boolean success True if the ready message was sent successfully
function cloude.ready() end
...
function cloude.add_browser_css(moduleItem, regexSelector) end
...
---@param regexSelector? string Regex pattern for URL matching (default: ".*")
---@return integer moduleId Module ID for later removal, or 0 on failure
function cloude.add_browser_js(moduleItem, regexSelector) end
...
function cloude.remove_browser_module(moduleId) end
...
function cloude.config.get(key) end
...
---@return string|nil error Error message if the call failed
function cloude.config.set(key, value) end
...
function cloude.config.on_change(key_or_callback, callback) end

---Get the version of Cloude in semantic versioning format
---@return string version Cloude version string (e.g., "1.0.0")
function cloude.version() end

---Get the path to the Steam directory
---@return string steamPath Full path to Steam installation directory
function cloude.steam_path() end

---Get the path to the Cloude install directory
---@return string installPath Full path to Cloude installation directory
function cloude.get_install_path() end
...
function cloude.get_plugin_logs() end
...
function cloude.call_frontend_method(methodName, params) end
...
function cloude.change_plugin_status(pluginName) end
...
function cloude.is_plugin_enabled(pluginName) end

--- Compare two semantic versions against one another. Very useful when you need to conditionally add features depending on the version of Cloude.
---
--- Example: If there is a API function "new_call" introduced in Cloude 2.30.0 and up, you can check if the function is available on the users installation with
---```lua
--- cloude.cmp_version(cloude.version(), "2.30.0") >= 0
--- ```
---
...
function cloude.cmp_version(version1, version2) end
