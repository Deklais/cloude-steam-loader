...
constexpr const char* kBootstrapTraceEnv = "CLOUDE_BOOTSTRAP_TRACE_PATH";
} // namespace
...
static const char* get_runtime_path()
{
    ...
    if (!initialized) {
        if (!get_adjacent_or_configured_path(kRuntimePathEnv, __CLOUDE_OUTPUT_NAME__, R_OK, runtime_path, sizeof(runtime_path))) {
            return nullptr;
        }
        initialized = true;
    }
...
static const char* get_child_hook_path()
{
    ...
    if (!initialized) {
        if (!get_adjacent_or_configured_path(kChildHookPathEnv, __CLOUDE_CHILD_HOOK_OUTPUT_NAME__, R_OK, child_hook_path, sizeof(child_hook_path))) {
            return nullptr;
        }
        initialized = true;
    }
...
static const char* get_hook_helper_path()
{
    ...
    if (!initialized) {
        if (!get_adjacent_or_configured_path(kHookHelperPathEnv, __CLOUDE_HOOK_HELPER_OUTPUT_NAME__, R_OK, hook_helper_path, sizeof(hook_helper_path))) {
            return nullptr;
        }
        initialized = true;
    }
...
static int cloude_execve(const char* path, char* const argv[], char* const envp[])
{
...
DYLD_INTERPOSE(cloude_execve, execve);
...
__attribute__((constructor)) static void bootstrap_init()
{
    ...
    if (!runtime_path) {
        fprintf(stderr, "[Cloude] Failed to resolve runtime path for macOS bootstrap.\n");
        return;
    }

    g_cloude_handle = dlopen(runtime_path, RTLD_NOW | RTLD_GLOBAL);
    if (!g_cloude_handle) {
        fprintf(stderr, "[Cloude] Failed to load runtime library: %s\n", dlerror());
        return;
    }

    start_cloude_t start_cloude = (start_cloude_t)dlsym(g_cloude_handle, "StartCloude");
    if (!start_cloude) {
        fprintf(stderr, "[Cloude] Failed to locate StartCloude: %s\n", dlerror());
    ...
    if (start_cloude() < 0) {
        fprintf(stderr, "[Cloude] Failed to start runtime.\n");
        dlclose(g_cloude_handle);
        g_cloude_handle = nullptr;
    }
}
...
__attribute__((destructor)) static void bootstrap_cleanup()
{
    ...
    stop_cloude_t stop_cloude = (stop_cloude_t)dlsym(g_cloude_handle, "StopCloude");
    if (stop_cloude) {
        stop_cloude();
    }
