...
constexpr const char* kDebugPortEnv = "CLOUDE_DEBUG_PORT";
constexpr const char* kRuntimePathEnv = "CLOUDE_RUNTIME_PATH";
constexpr const char* kHookHelperPathEnv = "CLOUDE_HOOK_HELPER_PATH";
constexpr const char* kChildHookPathEnv = "CLOUDE_CHILD_HOOK_PATH";
constexpr const char* kAssetsRootEnv = "CLOUDE_ASSETS_ROOT";
constexpr const char* kBrowserContextPathEnv = "CLOUDE_BROWSER_CONTEXT_PATH";
constexpr const char* kSteamExecutableEnv = "CLOUDE_STEAM_EXECUTABLE";
...
constexpr const char* kBootstrapDirectoryName = "cloude";
constexpr const char* kBootstrapLoaderDirectoryName = "loader";
constexpr const char* kBootstrapChunksDirectoryName = "chunks";
constexpr const char* kBootstrapFrontendModuleName = "cloude-frontend.js";
} // namespace
...
static void start_browser_context_probe()
{
    ...
    char temp_path[] = "/tmp/cloude-browser-context-XXXXXX";
...
static bool resolve_bootstrap_asset_sources(const char* wrapper_directory, std::filesystem::path* loader_source_dir, std::filesystem::path* frontend_source)
{
    ...
    auto has_valid_layout = [](const std::filesystem::path& candidate_root, std::filesystem::path* resolved_loader_dir, std::filesystem::path* resolved_frontend) -> bool
    {
        ...
        const std::filesystem::path loader_entry = loader_dir / "cloude.js";
    ...
    candidates.emplace_back(std::filesystem::path(wrapper_directory) / ".." / "Resources" / "cloude-assets");
...
#ifdef CLOUDE_ROOT
    const std::filesystem::path repo_root(CLOUDE_ROOT);
    ...
    if (std::filesystem::is_regular_file(legacy_loader_dir / "cloude.js", legacy_error) && !legacy_error &&
...
static bool prepare_macos_bootstrap_assets(const char* steam_executable_path)
{
    ...
    if (!sync_text_file(loader_source_dir / "cloude.js", loader_target_dir / "cloude.js")) {
        return false;
    }
...
int main(int argc, char** argv)
{
    ...
    if (!get_executable_path(current_path, sizeof(current_path))) {
        fprintf(stderr, "[Cloude] Failed to resolve wrapper path.\n");
        return 1;
    }
    ...
    snprintf(bootstrap_path, sizeof(bootstrap_path), "%s/%s", directory, __CLOUDE_BOOTLOADER_OUTPUT_NAME__);

    if (access(bootstrap_path, R_OK) != 0) {
        fprintf(stderr, "[Cloude] Missing bootstrap library at %s\n", bootstrap_path);
        return 1;
    }

    if (!ensure_adjacent_artifact_env(kRuntimePathEnv, directory, __CLOUDE_OUTPUT_NAME__)) {
        const char* configured_runtime = getenv(kRuntimePathEnv);
        if (!configured_runtime || configured_runtime[0] == '\0') {
            fprintf(stderr, "[Cloude] Missing Cloude runtime path. Set %s or place %s beside the launcher.\n", kRuntimePathEnv, __CLOUDE_OUTPUT_NAME__);
            return 1;
        }
    }

    if (!ensure_adjacent_artifact_env(kHookHelperPathEnv, directory, __CLOUDE_HOOK_HELPER_OUTPUT_NAME__)) {
        const char* configured_hhx = getenv(kHookHelperPathEnv);
        if (!configured_hhx || configured_hhx[0] == '\0') {
            fprintf(stderr, "[Cloude] Missing helper hook path. Set %s or place %s beside the launcher.\n", kHookHelperPathEnv, __CLOUDE_HOOK_HELPER_OUTPUT_NAME__);
            return 1;
        }
    }

    if (!ensure_adjacent_artifact_env(kChildHookPathEnv, directory, __CLOUDE_CHILD_HOOK_OUTPUT_NAME__)) {
        const char* configured_child_hook = getenv(kChildHookPathEnv);
        if (!configured_child_hook || configured_child_hook[0] == '\0') {
            fprintf(stderr, "[Cloude] Missing child hook path. Set %s or place %s beside the launcher.\n", kChildHookPathEnv, __CLOUDE_CHILD_HOOK_OUTPUT_NAME__);
            return 1;
        }
    }
    ...
    if (!resolve_steam_target(directory, target_steam_path, sizeof(target_steam_path))) {
        fprintf(stderr, "[Cloude] Failed to locate the real Steam runtime. Set %s or reinstall Steam.\n", kSteamExecutableEnv);
        return 1;
    }
    ...
    remove_cef_remote_debugging_marker(target_steam_path);
    fprintf(stderr, "[Cloude] Failed to launch Steam: %s\n", strerror(errno));
    return 1;
}
#endif
