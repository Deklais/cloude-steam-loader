...
const EvaluatePatches = (activeTheme: ThemeItem, documentTitle: string, classList: string[], document: Document, popup: any) => {
	(activeTheme.data.Patches ?? []).forEach((patch: Patch) => {
		...
		// backwards compatibility with old cloude versions.
...
export function addWindowBreadcrumbs(windowContext: any) {
	...
	if (windowContext?.m_rgParams?.minHeight === 601 && windowContext?.m_rgParams?.minWidth === 842) {
		addBreadCrumb('CloudeWindow_GameProperties');
	}
}
...
export function patchDocumentContext(windowContext: any) {
	...
	DOMModifier.AddStyleSheetFromText(document, pluginSelf.quickCss, 'CloudeQuickCss');

	for (const plugin of pluginSelf.enabledPlugins) {
		document.documentElement.classList.add(plugin);
	}

	document.documentElement.setAttribute('data-cloude-plugin', pluginSelf?.enabledPlugins?.join(' '));
...
export async function onWindowCreatedCallback(windowContext: any): Promise<void> {
	...
	/** Patch the steam root menu to add the Cloude root menu */
	...
	if (!prevMainWindow && pluginSelf.mainWindow) {
		window.dispatchEvent(new CustomEvent('cloude-main-window-ready'));
	}
