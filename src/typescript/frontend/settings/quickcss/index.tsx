...
import { ffi, DialogButton, DialogControlsSection, DialogHeader, Field, IconsModule, Cloude, pluginSelf, Toggle } from '@steambrew/client';
...
import { setEditorCode, setIsCloudeOpen, setIsWatching, useQuickCssState } from '../../utils/quick-css-state';
...
function UpdateStylesLive(cssContent: string) {
	for (const popup of g_PopupManager.GetPopups()) {
		const quickCssStyles = popup?.window?.document?.getElementById('CloudeQuickCss');
		if (quickCssStyles) {
...
/* called from the cloude backend */
...
Cloude.exposeObj({ OnQuickCssFileChanged });

export const CloudeQuickCssEditor = () => {
	...
	const windowConfig = {
		...
		popup_class: 'fullheight CloudeQuickCss_Popup',
	...
	const { popup, element } = createWindowContext('CloudeDesktopPopup', windowConfig, windowHandler('Window_CloudeDesktop'));
		...
		ReactDOM.createPortal(
			<OwnerWindowRef ownerWindow={popup}>
				<div className="PopupFullWindow">
					<RenderWindowTitle popup={popup} onClose={setIsCloudeOpen.bind(null, false)} />
					<GenericConfirmDialog>
						...
						<form ref={containerRef} className="CloudeQuickCss_CodeEditor" />
					</GenericConfirmDialog>
					{activeMenu && (
	...
	return (
		<>
			...
			<Field label={locale.quickCssStatusLoaded} bottomSeparator="none" icon={<IconsModule.Checkmark color="green" />}>
				<DialogButton onClick={() => setIsCloudeOpen(true)} className={settingsClasses.SettingsDialogButton}>
