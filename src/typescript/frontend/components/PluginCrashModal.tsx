...
const CrashModal = ({ detail, closeModal, onResolve }: { detail: PluginCrashInfo; closeModal: () => void; onResolve: () => void }): React.ReactElement => {
	...
	const openCrashFolder = () => {
		const target = detail.crashDumpDir || `${pluginSelf?.steamPath ?? ''}/cloude/crashes`;
		if (target) {
	...
	const viewLogs = () => {
		...
		closeModal();
		Navigation.Navigate('/cloude/settings/logs');
	};
			...
			strDescription={
				<>
					<Field
						description={
							<>
								{formatString(locale.crashModalDescPre, pluginLabel)} {locale.crashModalDescPost}
							</>
						}
