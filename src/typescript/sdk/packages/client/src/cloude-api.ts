...
/*
 Global Cloude API for developers.
*/
...
type Cloude = {
	 ...
	 * ```typescript
	 * // before
	 * const result = await Cloude.callServerMethod('methodName', { arg1: 'value' });
	 * // after
	...
	/**
	 * ```typescript
	 * Cloude.openQuickAccess(); // open QAM home
	 * Cloude.openQuickAccess({ theme: 'themeName' }); // "name" property from the themes skin.json
	 * Cloude.openQuickAccess({ plugin: 'pluginName' }); // "name" property from the plugins plugin.json
	 * ```
	 * @param target
	 * @returns
	 */
...
const CDP_PROXY_BINDING = '__cloude_cdp_proxy__';
const CDP_EXTENSION_BINDING = '__cloude_extension_route__';
const CDP_EXT_RESP = 'CLOUDE_CHROME_DEV_TOOLS_PROTOCOL_DO_NOT_USE_OR_OVERRIDE_ONMESSAGE';
...
declare global {
	interface Window {
		...
		__cloude_cdp_resolve__: (callbackId: number, result: any) => void;
		__cloude_cdp_reject__: (callbackId: number, error: string) => void;
		__cloude_cdp_event__: (data: any) => void;
	}
}
...
function initializeCDPBus() {
	...
	window.__cloude_cdp_resolve__ = (callbackId: number, result: any) => {
	...
	window.__cloude_cdp_reject__ = (callbackId: number, error: string) => {
		const pending = _pending.get(callbackId);
		if (pending) {
			_pending.delete(callbackId);
	...
	window.__cloude_cdp_event__ = (data: any) => {
...
export class CloudeChromeDevToolsProtocol {
...
/* backwards compat with old callers (without requiring recompile with new @steambrew/ttc version). falls back to Cloudes internal CDP. */
class CloudeChromeDevToolsProtocolShared extends CloudeChromeDevToolsProtocol {
	constructor() {
		super('__cloude_internal__');
	}
...
const ChromeDevToolsProtocol: CloudeChromeDevToolsProtocol = new CloudeChromeDevToolsProtocolShared();
const Cloude: Cloude = window.Cloude;
export { BindPluginSettings, callable, ffi, ChromeDevToolsProtocol, Cloude, pluginConfig, subscribePluginConfig, usePluginConfig };
