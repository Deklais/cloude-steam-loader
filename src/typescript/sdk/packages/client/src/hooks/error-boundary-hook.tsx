...
import CloudeErrorBoundary from '../components/CloudeErrorBoundary';
...
class ErrorBoundaryHook extends Logger {
	...
	constructor() {
		...
		this.errorBoundaryPatch = replacePatch(ErrorBoundary.prototype, 'render', function (this: any) {
			if (this.state._cloudeForceRerender) {
				const stateClone = { ...this.state, _cloudeForceRerender: null };
		...
		ErrorBoundary.prototype._cloudeForceRerender = function (this: any) {
			this.setState({ ...this.state, _cloudeForceRerender: true });
		};
	}
