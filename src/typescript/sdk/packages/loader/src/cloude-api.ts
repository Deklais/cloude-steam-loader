...
declare global {
	interface Window {
		PLUGIN_LIST: any;
		CLOUDE_BACKEND_IPC: typeof backendIPC;
		CLOUDE_IPC_SOCKET: WebSocket;
		CURRENT_IPC_CALL_COUNT: number;
		CLOUDE_PLUGIN_SETTINGS_STORE: any;
		CLOUDE_API: any;
		CLOUDE_PRIVATE_INTERNAL_FOREIGN_FUNCTION_INTERFACE_DO_NOT_USE: FFI_Binder;
	}
}
...
const backendIPC = {
	postMessage: async (messageId: number, contents: any): Promise<any> => {
		...
		const ffi = window.CLOUDE_PRIVATE_INTERNAL_FOREIGN_FUNCTION_INTERFACE_DO_NOT_USE;
		if (!ffi) return { returnValue: null };
		return ffi.call(messageId, contents.pluginName ?? '', '', []);
	},
};

window.CLOUDE_BACKEND_IPC = backendIPC;
...
class FFI_Binder {
	...
	async call(payloadType: IpcMethod, pluginName: string, methodName: string | number, argumentList: any, callSite?: Error, raw?: boolean): Promise<any> {
		if (typeof (window as any).__private_cloude_ffi_do_not_use__ !== 'function') {
			console.error("Cloude FFI is not available in this context. To use the FFI, make sure you've selected the 'cloude' context");
			return;
		}
		...
		return new Promise((resolve, reject) => {
			...
			(window as any).__private_cloude_ffi_do_not_use__(
				JSON.stringify({
					id: payloadType,
					call_id: requestId,
					caller,
	...
	__handleResponse(requestId: number, response: ResponsePayload) {
		...
		if (!pending) {
			console.warn('[Cloude] No pending request for', requestId, response);
			return;
		}
		...
		if (response.success) {
			...
			FFI_Binder.rejectWithCallSite(pending, 'Cloude Error: ' + (response.returnJson || 'Unknown error'));
		}
	}
...
window.CLOUDE_PRIVATE_INTERNAL_FOREIGN_FUNCTION_INTERFACE_DO_NOT_USE = ffiBinder;
...
export const Cloude = {
	...
	openQuickAccess: async (target?: { plugin: string } | { theme: string }): Promise<void> => {
		...
		(window.CLOUDE_API as any).openQuickAccess(target);
	},
...
export function callable(pluginNameOrFn: string | ((...args: any[]) => any), route: string) {
	...
	return (...args: any[]) => {
		...
		return Cloude.callServerMethod(pluginNameOrFn, route, args[0], callSite);
	};
}
...
export const BindPluginSettings = (pluginName: string) => window.CLOUDE_PLUGIN_SETTINGS_STORE?.[pluginName]?.settingsStore;

type ConfigChangeCallback = (key: string, value: any) => void;
const _pluginConfigListeners = new Map<string, Set<ConfigChangeCallback>>();

(window as any).__cloude_plugin_config_changed__ = (pluginName: string, key: string, valueJson: string) => {
	try {
		...
		console.error('[Cloude] Config change error:', e);
	}
};
...
const _frontendApiReady: Promise<void> = new Promise((resolve) => {
	if (typeof window.CLOUDE_API?.openQuickAccess === 'function') {
		resolve();
	} else {
		window.addEventListener('cloude-quick-access-ready', () => resolve(), { once: true });
	}
});
