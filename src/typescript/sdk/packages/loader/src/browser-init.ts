...
export async function appendAccentColor() {
	const systemColors: SystemColors = JSON.parse(await Cloude.callServerMethod('core', 'Core_GetSystemColors'));
...
export async function appendQuickCss() {
	try {
		const quickCss: string = JSON.parse(await Cloude.callServerMethod('core', 'Core_LoadQuickCss'));
		addStyleSheetFromText(await waitForDocument(), quickCss, 'CloudeQuickCss');
	} catch {}
}

export async function addPluginDOMBreadCrumbs(enabledPlugins: string[] = []) {
	const doc = await waitForDocument();
	doc.documentElement.setAttribute('data-cloude-plugin', enabledPlugins.join(' '));
	doc.documentElement.classList.add('CloudeWindow_SteamBrowser');
}
