...
readonly REQUIRED_RUNTIME_ARTIFACTS=(
    "libcloude.dylib"
    "libcloude_hhx64.dylib"
)
readonly OPTIONAL_RUNTIME_ARTIFACTS=(
    "libcloude_child_hook.dylib"
)
readonly WRAPPER_APP_NAME="Steam Cloude.app"
readonly WRAPPER_EXECUTABLE_NAME="steam_osx"
readonly WRAPPER_BOOTSTRAP_ARTIFACT="libcloude_bootstrap.dylib"
readonly WRAPPER_RUNTIME_ARTIFACTS=(
    "libcloude.dylib"
    "libcloude_hhx64.dylib"
    "libcloude_child_hook.dylib"
)
readonly WRAPPER_ASSET_RELATIVE_DIR="cloude-assets"
...
readonly DEFAULT_RUNTIME_INSTALL_ROOT="${HOME}/Library/Application Support/Cloude/runtime"
...
readonly STEAM_CFG_BACKUP_NAME="Steam.cfg.cloude.backup"
readonly STEAM_CFG_STATE_NAME="Steam.cfg.cloude.state"
readonly STEAM_CFG_MANAGED_MARKER="# Added by Cloude install_macos.sh"
...
3. Launch Cloude by opening that app bundle
...
copy_wrapper_assets_into_runtime() {
    ...
    local loader_entry="${REPO_ROOT}/src/typescript/sdk/packages/loader/build/cloude.js"
    ...
    install_data_file "${loader_entry}" "${loader_target_dir}/cloude.js"
    install_data_file "${frontend_source}" "${asset_root}/cloude-frontend.js"
...
is_tier0_proxy() {
    local target="$1"
    [ -f "${target}" ] || return 1
    nm -gU "${target}" 2>/dev/null | rg -q "cloude_tier0_proxy_marker"
}
...
strip_managed_steam_cfg_lines() {
    ...
    awk '
        ...
        /^# Added by Cloude install_macos\.sh$/ { next }
        /^# Added by Cloude install_macos_tier0\.sh$/ { next }
        { print }
...
configure_steam_cfg_inhibit_all() {
    ...
    local stripped_cfg
    stripped_cfg=$(mktemp /tmp/cloude-steamcfg-stripped.XXXXXX)
    local merged_cfg
    merged_cfg=$(mktemp /tmp/cloude-steamcfg-merged.XXXXXX)
    ...
    case "${state_value}" in
        ...
        absent)
            if [ -f "${steam_cfg}" ]; then
                ...
                local stripped_cfg
                stripped_cfg=$(mktemp /tmp/cloude-steamcfg-restore.XXXXXX)
...
install_or_repair() {
    ...
    local temp_dir
    temp_dir=$(mktemp -d /tmp/cloude-tier0-proxy.XXXXXX)
    ...
    if [ "${DRY_RUN}" -eq 0 ] && ! is_tier0_proxy "${STEAM_DIR}/${TIER0_NAME}"; then
        fail "Installed ${TIER0_NAME} is not the Cloude tier0 proxy."
    fi
...
uninstall_runtime() {
    ...
    log "Restored original ${TIER0_NAME} and removed Cloude runtime artifacts."
}
