...
Launches Steam on macOS through Cloude's wrapper flow:
...
resolve_bootstrap_asset_paths() {
    ...
    local loader_candidates=(
        "${REPO_ROOT}/src/typescript/sdk/packages/loader/build/cloude.js"
    )
...
export CLOUDE_STEAM_EXECUTABLE="${STEAM_EXECUTABLE}"
export CLOUDE_RUNTIME_PATH="${runtime_path}"
export CLOUDE_HOOK_HELPER_PATH="${hook_helper_path}"
export CLOUDE_CHILD_HOOK_PATH="${child_hook_path}"

if [ -n "${EXPLICIT_NATIVE_CORNERS}" ]; then
    export CLOUDE_MACOS_NATIVE_CORNERS="${EXPLICIT_NATIVE_CORNERS}"
fi

if [ -n "${EXPLICIT_NATIVE_CORNER_RADIUS}" ]; then
    export CLOUDE_MACOS_NATIVE_CORNER_RADIUS="${EXPLICIT_NATIVE_CORNER_RADIUS}"
fi
