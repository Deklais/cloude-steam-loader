...
set(TS_ROOT    "${CLOUDE_BASE}/src/typescript")
set(TS_STAMPS  "${CMAKE_BINARY_DIR}/ts_stamps")
set(TS_BUILDER "${CLOUDE_BASE}/scripts/cmake/typescript_build.cmake")
...
file(GLOB _src_locales CONFIGURE_DEPENDS
    "${CLOUDE_BASE}/src/locales/*.json"
)

macro(_ts_package _name _dir)
  set(_stamp "${TS_STAMPS}/${_name}.stamp")
  file(RELATIVE_PATH _reldir "${CLOUDE_BASE}" "${_dir}")
  add_custom_command(
        OUTPUT  "${_stamp}"
        COMMAND ${CMAKE_COMMAND}
            -D "BUN=${BUN_EXECUTABLE}"
...
set(_virtfs_h "${CLOUDE_BASE}/src/include/cloude/virtfs.h")
if(WIN32 AND (CMAKE_BUILD_TYPE STREQUAL "Release" OR CMAKE_BUILD_TYPE STREQUAL "RelWithDebInfo"))
  set(_virtfs_outputs "${_virtfs_h}" "${CLOUDE_BASE}/scripts/resources.rc")
else()
...
add_custom_command(
    ...
    COMMAND ${CMAKE_COMMAND}
        ...
        -P "${CLOUDE_BASE}/scripts/cmake/generate_virtfs.cmake"
    DEPENDS
        "${TS_STAMPS}/api.stamp"
